...
fn write_report_header(out: &mut String, report: &SuiteReport<'_>) {
    ...
    let _ = writeln!(
        out,
        "This report is generated by `cargo run --release -p lyng-test262 -- --report {}`.",
        report.report_path
    );
    let _ = writeln!(
        out,
...
mod tests {
    ...
    fn report_documents_top_level_grouping_and_lists_skips_separately() {
        ...
        let manifest = ExclusionManifest {
            path: "reports/lyng/test262-exclusions.txt".to_string(),
            rules: Vec::new(),
        };
...
",
    ...
    fn checked_in_whole_report_has_only_manifest_skips() {
        assert_checked_in_report_has_only_manifest_skips("reports/lyng/test262.md");
    }

    #[test]
    fn checked_in_annexb_report_has_no_skips() {
        let report_path = workspace_root().join("reports/lyng/test262-annexb.md");
        let report = fs::read_to_string(&report_path).unwrap_or_else(|error| {
            panic!("{} should be readable: {error}", report_path.display())
        });
        assert!(
