...
impl Layout {
    ...
    /// argument order.
    const fn operand_names(self) -> &'static [&'static str] {
        match self {
            Self::None => &[],
            Self::A => &["a"],
            Self::Ab => &["a", "b"],
...
#[allow(
    ...
    reason = "header generation is a single ordered template whose line count mirrors the emitted file prologue"
)]
...
const fn prefix_kind_for(layout: Layout, length: u32) -> Option<PrefixKind> {
    match (layout, length) {
        (Layout::Abc, 4) | (Layout::AbcSlot, 6) => Some(PrefixKind::Abc),
        (Layout::Abx, 4 | 6) => Some(PrefixKind::Abx),
        _ => None,
    }
}

/// Whether the stub's bytecode form carries a feedback slot.
const fn is_profiled_for(layout: Layout, length: u32) -> bool {
...
#[allow(
    clippy::too_many_lines,
    reason = "wide-form dispatcher generation is an ordered opcode table template; splitting it would obscure emitted-code order"
)]
