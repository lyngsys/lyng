...
pub const DEFAULT_REPORT_PATH: &str = "reports/lyng/external-engine-compare.md";
pub const DEFAULT_JSON_PATH: &str = "reports/lyng/external-engine-compare.json";
...
fn parse_options(args: &[String]) -> CompareResult<Options> {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            "--lyng" => {
                set_engine_executable(&mut options, "lyng", next_string("--lyng", args.next())?);
            }
            "--qjs" => {
...
fn usage() -> String {
    "Usage: lyng-bench compare [--corpus <synthetic|v8-v7>] [--filter <name>] [--full-suite] [--preset <smoke|baseline|profile-target>] [--report <path>] [--json <path>] [--samples <n>] [--warmup-samples <n>] [--loop-trips <n>] [--timeout-ms <n>] [--scripts-dir <path>] [--lyng <path>] [--qjs <path>] [--jsc <path>]"
        .to_string()
}
...
fn default_engines() -> Vec<EngineConfig> {
    vec![
        EngineConfig::new("lyng", "target/release/lyng", []),
...
fn default_scripts_dir() -> PathBuf {
    PathBuf::from("/tmp/lyng-bench-compare-scripts")
}
...
fn run_engine_once(
    ...
    if engine.name == "lyng" && workload.requires_lyng_shell {
        command.arg("--shell");
    }
...
fn render_report(options: &Options, reports: &[EngineWorkloadReport]) -> String {
    ...
    let _ = writeln!(
        out,
        "This report is generated by `cargo run --release -p lyng-bench -- compare --report {}`.",
        options.report_path
    );
    let _ = writeln!(
        out,
...
mod tests {
    ...
    fn smoke_preset_configures_quick_external_engine_loop() {
        ...
        assert_eq!(options.engines[0].name, "lyng");
        assert_eq!(options.engines[0].executable, "/tmp/lyng");
    ...
    fn report_documents_quickjs_policy_ratios_and_profiler_commands() {
        ...
        assert!(markdown.contains("| `arithmetic-loop` | `arithmetic-control-flow` | `lyng` |"));
        assert!(markdown.contains("2.00x"));
        assert!(markdown.contains("target/release/lyng /tmp/compare-scripts/arithmetic-loop.js"));
        ...
        assert!(markdown.contains(
            "sample \"$pid\" 10 -file /tmp/lyng-compare-lyng-arithmetic-loop.sample.txt"
        ));
    ...
    fn json_report_records_engine_metadata_samples_and_quickjs_ratios() {
        ...
        assert_eq!(json["engines"][0]["name"], "lyng");
        assert_eq!(json["engines"][1]["pre_args"][0], "--script");
        assert_eq!(json["results"][0]["engine"], "lyng");
