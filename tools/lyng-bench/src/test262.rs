...
impl Test262Options {
    ...
    pub fn default_for_test() -> Self {
        Self {
            ...
            filter: None,
            manifest_path: "reports/lyng/test262-exclusions.txt".to_string(),
            no_skip: false,
...
fn usage() -> String {
    [
        "Usage: lyng-bench test262 [--preset <smoke|inner-loop|baseline|ci-regression|profile-target>] [--filter <path-or-fragment>] [--report <path>] [--json <path>] [--mode hybrid|scan|sample] [--samples <n>] [--warmup-samples <n>] [--sample-files <n>] [--manifest <path>] [--proposal-stage <4|3|2.7>] [--no-skip] [--timeout-ms <N>] [--profile-loop-ms <N>] [--print-counters] [-j <N>]",
        "",
...
pub fn render_markdown_report(
    ...
    let _ = writeln!(
        out,
        "This report is generated by `cargo run --release -p lyng-bench -- test262 --report {}`.",
        options.report_path
    );
    let _ = writeln!(
        out,
