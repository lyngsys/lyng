...
- **Compact asm-visible entry content.** Only fields read or written by LLInt hot paths stay inside each `FeedbackEntry`; richer Phase 3f state remains in the legacy side table. Existing IC fast-path evidence (`reports/lyng/phase-3f-status.md`) carries forward.
...
- **Verify the single-implementation invariant via the manifest (§10 DSL-0a):**
  ...
  - Test 7: opcode-counter mode (when compiled with `--features diagnostic-counters`) produces correct per-opcode dispatch counts under the DSL substrate, matching alpha-path counts within a documented per-handler instrumentation delta.
...
| `FeedbackVector` flat-array refactor regresses Phase 3f's packed sidecars | low-medium | medium | Refactor keeps rich monomorphic/proto/polymorphic state in the legacy side table and projects only compact LLInt headers into the flat array. Existing IC tests + benchmarks gate the refactor. |
...
12. **Opcode-counter mode asm overhead.** When `--features diagnostic-counters` is on, the proc-macro emits an extra ~3-instr counter-increment per handler. Quantify the per-bench overhead. If it's >5% on tight loops, consider a sparse counter that increments on a sampled subset of dispatches.
