...
- `PrimitiveHeap::{alloc_string, alloc_latin1_concat_string, alloc_utf16_concat_string}` in
  `crates/gc/src/arena.rs`: low-level string record allocation, including flat
  side payloads and cons-string child refs. Barrier: init-only.
- `PrimitiveHeap::{alloc_symbol, alloc_bigint, alloc_value_cell}` in
  `crates/gc/src/arena.rs`: low-level symbol, bigint, and mutable value-cell
  allocation. Barrier: init-only.
- `PrimitiveHeap::{alloc_object, alloc_function_payload, alloc_object_slots}` in
  `crates/gc/src/arena.rs`: low-level object record, function payload, and
  object slot-buffer allocation. Barrier: init-only.
- `PrimitiveHeap::{alloc_suspended_execution, alloc_suspended_registers}` in
  `crates/gc/src/arena.rs`: generator/async suspension records and copied register
  buffers. Barrier: init-only.
- `PrimitiveHeap::{alloc_environment, alloc_environment_slots}` in
  `crates/gc/src/arena.rs`: low-level environment records and dense binding slots.
  Barrier: init-only.
- `PrimitiveHeap::{alloc_code, alloc_code_slots}` in `crates/gc/src/arena.rs`:
  installed code records and constant slots. Barrier: init-only.
- `PrimitiveHeap::{alloc_realm, alloc_shape}` in `crates/gc/src/arena.rs`:
  low-level realm and shape records. Barrier: init-only.
- `PrimitiveMutator::alloc_*` in `crates/gc/src/mutator.rs`: public allocation
...
- `ValueSlotAllocator::allocate` in `crates/gc/src/arena/storage.rs`: allocates
...
- `ObjectRuntime::{alloc_shape, root_shape, transition_shape}` in
  `crates/objects/src/runtime.rs`: shape allocation and side metadata creation.
...
- `ObjectRuntime::{alloc_object, alloc_date_object}` in
  `crates/objects/src/runtime.rs`: object allocation owner that allocates named
...
- `ObjectAllocation` and `FunctionObjectData` builder methods in
  `crates/objects/src/object_records.rs` and
  `crates/objects/src/functions.rs`: pre-allocation request data. Barrier: init-only.
- Private/shared class storage allocation in `crates/objects/src/private_storage.rs`:
...
- `Agent::alloc_runtime_string`, `alloc_string_for_atom`, Latin-1 short-string caches, and
  two-code-unit string cache in `crates/env/src/agent/symbols.rs`: agent-level
...
- `Agent::{alloc_declarative_environment, alloc_private_environment,
  ...
  alloc_object_environment}` in `crates/env/src/agent/environments.rs`: environment
  record allocation wrappers over `PrimitiveMutator`. Barrier: init-only.
- `Agent::alloc_environment_layout` in `crates/env/src/agent/environments.rs`:
  layout metadata allocation. It stores atoms and flags, not heap object edges. Barrier:
  no-root.
- Promise side-table allocation in `crates/env/src/promise.rs`: promise,
  ...
  storage.
- Disposal side-table allocation in `crates/env/src/disposal.rs`: disposal
...
- `Vm::install_functions` and `Vm::install_constants` in
  `crates/vm/src/vm/install.rs`: installed code allocation, code constants, and
  parent-code backpatching. Barrier: yes for parent-code mutation; init-only for code
  constants allocated before publication.
- String and bigint helpers in `crates/vm/src/vm/values.rs` and
  `crates/vm/src/extensions.rs`: VM-facing wrappers over string and bigint
  allocation. Barrier: init-only.
- `Vm::snapshot_suspended_execution` in `crates/vm/src/vm/generators.rs`: copies
...
- Feedback vector allocation in `crates/vm/src/vm/feedback.rs`: VM side table keyed
...
- `ValueSlotAllocator::write` in `crates/gc/src/arena/storage.rs` writes one
...
- `set_function_payload_home_object`, `set_function_payload_environment`, and
  `set_function_payload_private_env` in `crates/gc/src/arena/store_helpers.rs`
  route typed handle writes through `HeapWriter::write_ref`. Barrier: yes.

### Symbol And Value-Cell Records

- `set_symbol_description` in `crates/gc/src/arena/store_helpers.rs` routes the
  description handle through `HeapWriter::write_ref`. Barrier: yes.
- `set_value_cell_value` and `set_value_cell_linked_string` in
  `crates/gc/src/arena/store_helpers.rs` route value and linked-string writes
...
- `set_object_prototype`, `set_object_shape`, `set_object_named_slots`,
  ...
  `crates/gc/src/arena/store_helpers.rs` route typed handle writes through
...
- `set_environment_outer`, `set_environment_function_object`, `set_environment_this_value`,
  ...
  `crates/gc/src/arena/store_helpers.rs` route typed handle and `Value` writes
  through `HeapWriter`. Barrier: yes.
- `Agent::{init_environment_slot, set_environment_slot, set_function_this_binding,
  ...
  `crates/env/src/agent/environments.rs` route environment slot and function-env
...
- `set_realm_global_object`, `set_realm_global_env`, `set_realm_bootstrap_code`, and
  `set_realm_root_shape` in `crates/gc/src/arena/store_helpers.rs` route typed
...
- `WeakMapState::set` in `crates/gc/src/weak.rs` writes `WeakHeapRef -> Value`
  entries. Barrier: weak-special.
- `WeakSetState::insert` in `crates/gc/src/weak.rs` writes weak heap refs.
  Barrier: weak-special.
- `WeakRefState` and finalization-registry state are initialized and mutated through
  `PrimitiveMutator` methods in `crates/gc/src/mutator.rs` and
  `crates/gc/src/arena/weak_state.rs`. Barrier: weak-special.

## Object And Shape Side-Metadata Write Sites

- `NamedPropertyDictionary::upsert` in `crates/objects/src/object_metadata.rs`
...
- `Agent::set_module_binding_alias` and `Agent::global_set_lexical_binding` in
  `crates/env/src/agent/environments.rs` write environment refs into environment
  side metadata. Barrier: no-root today; if metadata becomes heap-owned, these become
  `Barrier: yes`.
- Promise mutation methods in `crates/env/src/promise.rs` write promise results,
...
- Disposal mutation methods in `crates/env/src/disposal.rs` write disposable
  resource records, pending errors, and object-to-record indexes. Barrier: no-root today
  through `AgentDisposalTables` tracing.
- `Agent::keep_weak_target_alive` in `crates/env/src/agent/weak_finalization.rs`
...
- `NamedPropertyFeedback` and `KeyedPropertyFeedback` mutations in
  `crates/vm/src/vm/feedback.rs` write object/shape cache entries and dense-index
...
- `Vm::ensure_feedback_site_execution` in `crates/vm/src/vm/feedback.rs` allocates
...
crates/gc/src/weak.rs	self[.]entries[.]insert[(]key, value[)];
crates/gc/src/weak.rs	self[.]entries[.]insert[(]value[)];
crates/objects/src/object_metadata.rs	self[.]entries[.]insert[(]key, entry[)];
crates/objects/src/runtime.rs	self[.]root_shapes[.]insert[(]key, id[)];
crates/objects/src/runtime.rs	properties[.]push[(]property[)];
crates/objects/src/runtime_storage.rs	entries[.]insert[(]index, SparseElementEntry::new[(]payload, attrs[)][)];
crates/objects/src/functions.rs	self[.]environment = environment;
crates/env/src/module_records.rs	self[.]code = code;
crates/env/src/module_records.rs	self[.]environment = environment;
crates/env/src/module_records.rs	self[.]namespace = namespace;
crates/env/src/module_records.rs	self[.]deferred_namespace = namespace;
crates/env/src/module_records.rs	self[.]import_meta_object = import_meta_object;
crates/env/src/module_records.rs	self[.]resolved_exports = resolved_exports;
crates/env/src/module_records.rs	self[.]evaluation_error = evaluation_error;
crates/env/src/promise.rs	self[.]promises[.]push[(]Some[(]PromiseRecord::new[(]object, realm[)][)][)];
crates/env/src/promise.rs	self[.]promise_by_object\[index\] = Some[(]id[)];
crates/env/src/promise.rs	record[.]result = value;
crates/env/src/promise.rs	record[.]result = reason;
crates/env/src/promise.rs	self[.]reactions[.]push[(]Some[(]reaction[)][)];
crates/env/src/promise.rs	self[.]capabilities[.]push[(]Some[(]PromiseCapabilityRecord::new[(][)][)][)];
crates/env/src/promise.rs	record[.]promise = Some[(]promise[)];
crates/env/src/promise.rs	record[.]resolve = Some[(]resolve[)];
crates/env/src/promise.rs	record[.]reject = Some[(]reject[)];
crates/env/src/promise.rs	self[.]resolving_functions[.]push[(]Some[(]record[)][)];
crates/env/src/promise.rs	self[.]resolving_function_by_object\[index\] = Some[(]id[)];
crates/env/src/promise.rs	self[.]finally_functions[.]push[(]Some[(]record[)][)];
crates/env/src/promise.rs	self[.]finally_function_by_object\[index\] = Some[(]id[)];
crates/env/src/promise.rs	self[.]combinator_elements[.]push[(]Some[(]record[)][)];
crates/env/src/promise.rs	self[.]combinator_element_by_object\[index\] = Some[(]id[)];
crates/env/src/disposal.rs	self[.]capability_by_object\[index\] = Some[(]capability[)];
crates/env/src/disposal.rs	record[.]resources[.]push[(]resource[)];
crates/env/src/disposal.rs	record[.]pending_error = pending_error;
crates/env/src/disposal.rs	self[.]async_resumes[.]push[(]Some[(]record[)][)];
crates/env/src/disposal.rs	self[.]async_resume_by_object\[index\] = Some[(]id[)];
crates/env/src/agent/environments.rs	self[.]environment_layouts[.]push[(]Some[(]layout[)][)];
crates/env/src/agent/environments.rs	[*]target = alias;
crates/env/src/agent/environments.rs	lexical_bindings[.]push[(]binding[)];
crates/env/src/agent/weak_finalization.rs	self[.]kept_objects[.]push[(]target[)];
crates/vm/src/vm/feedback.rs	self[.]entries\[index\] = Some[(]plan[)];
crates/vm/src/vm/feedback.rs	self[.]entries\[usize::from[(]self[.]entry_count[)]\] = Some[(]plan[)];
crates/vm/src/vm/feedback.rs	self[.]entries\[0\] = Some[(]entry[)];
crates/vm/src/vm/feedback.rs	self[.]named_entries\[0\] = Some[(]KeyedNamedPropertyCacheEntry
crates/vm/src/vm/feedback.rs	self[.]named_entries\[0\] =
crates/vm/src/vm/feedback.rs	self[.]named_entries\[index\] =
crates/vm/src/vm/feedback.rs	self[.]named_entries\[usize::from[(]self[.]named_entry_count[)]\] =
crates/vm/src/vm/feedback.rs	self[.]dense_entries\[index\] = Some[(]plan[)];
crates/vm/src/vm/feedback.rs	self[.]dense_entries\[usize::from[(]self[.]dense_entry_count[)]\] = Some[(]plan[)];
crates/vm/src/vm/feedback.rs	self[.]dense_entries\[0\] = Some[(]entry[)];
crates/vm/src/vm/feedback.rs	self[.]feedback_vectors\[index\] =
