...
- [ ] Verify with `cargo test -p lyng-builtins --lib`, `cargo fmt --all --check`, and `cargo clippy -p lyng-builtins --lib --all-features -- -W clippy::pedantic`.
...
- [ ] Verify with `cargo test -p lyng-builtins --lib` and targeted `lyng-tests` bootstrap/runtime coverage.
- [ ] Add or update docs in `docs/lyng/builtin-bootstrap.md` when the module ownership changes.
...
- [ ] Verify with `cargo test -p lyng-ops`, `cargo test -p lyng-objects`, `cargo test -p lyng-vm`, and targeted `lyng-test262 --filter built-ins/Proxy`.
...
- [ ] Verify Function constructor, indirect eval, direct eval, and evalScript with `cargo test -p lyng-compiler`, `cargo test -p lyng-vm`, and targeted Test262 eval/function filters.
- [ ] Update `docs/lyng/dynamic-scope-and-eval.md` when the shared service API exists.
...
- [ ] Verify with `cargo test -p lyng-builtins --lib` and family-specific `lyng-tests`/Test262 slices for moved dispatch groups.
