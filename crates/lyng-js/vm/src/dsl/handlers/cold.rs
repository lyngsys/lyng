//! Cold DSL handlers. Originally auto-generated by `lyng-js-dsl-codegen`
//! (B46–B47); extended by hand during DSL-1 Phase 1.C with inline SMI
//! fast paths for selected hot opcodes (op_sub, op_mul, op_bit_and,
//! op_shift_left, op_shift_right, op_increment, op_decrement).
//!
//! Each Cold opcode has:
//! 1. A `#[unsafe(naked)] extern "C" fn op_xxx_dsl()` handler emitted
//!    by `llint_handler!`. Cold stubs delegate entirely to
//!    `op_xxx_slow_rs` via `call_slow! + dispatch_after_slow!`.
//!    Inline-ported opcodes additionally contain a direct SMI fast
//!    path that bails to a `.slow:` label on tag-check or overflow.
...
//!    fast path's tail to record SMI feedback through the legacy
//!    `vm.record_feedback_slot` bookkeeping.
//!
//! Cold stubs: re-run `cargo run -p lyng-js-dsl-codegen` to
//! regenerate after touching `tools/lyng-js-dsl-codegen/src/main.rs`.
//! Inline fast paths and `*_record_smi_rs` shims: hand-maintained;
//! see `docs/superpowers/plans/2026-05-21-dsl-1-phase-1c-smi-arith-and-bitwise.md`
//! and adjacent plans.

#[cfg(target_arch = "aarch64")]
use crate::{
    bit_and_smi, call_slow, check_smi, cmp_branch_eq, decode_a, decode_ab, decode_abc,
    decode_abc_slot, decode_abx, decode_ax, dispatch, dispatch_after_slow, load_acc,
    load_constant, load_local_fixed, load_reg, load_state_value, load_uninit_lex_sentinel,
    mul_smi_overflow, shift_left_smi, shift_right_smi, store_acc, store_local_fixed, store_reg,
    sub_smi_overflow, tag_bool_const, tag_null, tag_smi, tag_smi_const,
    tag_smi_from_signed_byte, tag_undefined, untag_smi,
};
...
/// Fast-path feedback-recording shim for `op_shift_left`. Mirrors
/// `op_add_record_smi_rs` in hot.rs: bumps the warmup counter,
/// allocates the legacy vector at threshold, mirrors legacy state to
/// the flat array, observes the tier feedback event. Returns
/// `Continue { pc_advance: 6 }` so the asm bridge advances PC by
...
/// Fast-path feedback-recording shim for `op_shift_right`. Mirrors
/// `op_add_record_smi_rs` in hot.rs: bumps the warmup counter,
/// allocates the legacy vector at threshold, mirrors legacy state to
/// the flat array, observes the tier feedback event. Returns
/// `Continue { pc_advance: 6 }` so the asm bridge advances PC by
