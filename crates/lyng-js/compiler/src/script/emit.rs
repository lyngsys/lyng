...
impl<'a, 'b> FunctionCompiler<'a, 'b> {
    ...
    pub(super) fn emit_push_with_env(&mut self, object: u16, span: Span) -> LoweringResult<()> {
        let instruction_offset = self.builder.emit_ax(Opcode::PushWithEnv, i32::from(object));
        self.attach_safepoint(instruction_offset, span, SafepointKind::Allocation);
        Ok(())
    }

    pub(super) fn emit_pop_with_env(&mut self) {
    ...
    pub(super) fn emit_logical_assignment_short_circuit(
        ...
        match operator {
            ...
            AssignOp::NullishAssign => {
                ...
                let jump_assign_from_null = self
                    .builder
    ...
    pub(super) fn constant_atom(&mut self, atom: AtomId) -> u32 {
    ...
    pub(super) fn constant_smi(&mut self, value: i32) -> u32 {
        self.builder.add_constant(ConstantValue::Smi(value))
    }

    pub(super) fn constant_float(&mut self, bits: u64) -> u32 {
        if let Some(index) = self.float_constants.get(&bits) {
            return *index;
        }
        let index = self.builder.add_constant(ConstantValue::Float64Bits(bits));
        self.float_constants.insert(bits, index);
        index
    }

    pub(super) fn constant_builtin(&mut self, builtin: BuiltinFunctionId) -> u32 {
    ...
    pub(super) fn attach_safepoint(
        ...
        let register_window_len = self
            ...
            .expect("register window should fit in u16");
