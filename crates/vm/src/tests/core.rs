...
#[cfg(feature = "opcode-counters")]
#[test]
fn simple_nested_call0_return_avoids_semantic_slow_path() {
    ...
    assert_eq!(
        ...
        0,
        "eligible Call0 should enter the callee without the semantic slow bridge"
    );
    assert_eq!(
        slow_path.semantic(Opcode::Return),
        1,
        "the nested function Return should avoid the semantic slow bridge; only the top-level script Return exits through it"
    );
}
...
fn simple_tail_call_avoids_semantic_slow_path() {
    ...
    assert_eq!(
        ...
        0,
        "eligible TailCall should replace the active frame without the semantic slow bridge"
    );
}
