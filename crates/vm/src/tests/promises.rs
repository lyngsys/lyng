...
fn evaluate_script_drains_nested_promise_jobs_to_quiescence() {
    ...
    let result = vm
        .script_eval(agent, realm, &unit).with_host(&host).with_registry(&mut registry).run()
        .unwrap();
...
fn evaluate_script_promise_reaction_can_update_global_lexical_binding() {
    ...
    let result = vm
        .script_eval(agent, realm, &readback).with_host(&host).with_registry(&mut registry).run()
...
fn evaluate_script_top_level_closures_share_rebound_lexical_binding() {
    ...
    let result = vm
        .script_eval(agent, realm, &unit).run()
