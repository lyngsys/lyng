...
//!
//! - [`hot`]   — populated by B39–B42 (hottest opcodes: SMI arith,
//!   register moves, object access).
...
//!
//! [`DSL_DISPATCH_TABLE`] is finalized in B47 — every Cold opcode now
//! points at its generated `op_xxx_dsl` stub from [`cold`], the 5 Hot
//! opcodes point at [`hot`], and the 7 Warm opcodes point at [`warm`].
//! Opcode bytes outside the 152-variant range (153..256) keep the
//! `unimplemented_dsl_handler` placeholder.
//!
//! The table is still **not reachable at runtime** — `Vm::run` continues
//! to route through `run_via_trampoline`. Phase C (DSL-0c) flips
...
/// Diverging placeholder for opcode byte slots that have no real
/// handler — currently bytes 153..256, since the bytecode enum tops
...
// DSL dispatch table.
//
// One slot per opcode byte (0..256). Slot 0..152 carry real handlers,
// derived from the `OPCODES` manifest in
// `crate::dsl::opcode_manifest`. The mapping is hand-written rather
// than computed from the manifest because:
//
// 1. `static` array initializers can't run loops in const context.
// 2. The proc-macro emits `unsafe extern "C" fn op_xxx_dsl() -> !`
//    symbols directly — no indirection to walk at const-eval time.
// 3. The compiler verifies the function-pointer type matches
//    `DslHandler` at write time; mismatches surface as a compile error
...
// `crates/vm/tests/dsl_dispatch_table.rs` walks the manifest
// against this table at link time to catch drift.
// =====================================================================

/// One slot per opcode byte. Indexed by `Opcode as u8`. Phase C flips
/// `run_dsl_trampoline` to dispatch through this table; until then
/// it's link-validated but unreachable at runtime.
#[allow(dead_code)]
#[cfg(target_arch = "aarch64")]
pub static DSL_DISPATCH_TABLE: [DslHandler; 256] = build_dispatch_table();

/// Non-aarch64 hosts get the all-placeholder table. The asm
/// trampoline is aarch64-only per design §3.
#[allow(dead_code)]
#[cfg(not(target_arch = "aarch64"))]
pub static DSL_DISPATCH_TABLE: [DslHandler; 256] = [unimplemented_dsl_handler; 256];

/// Build the dispatch table at const-eval time. One match arm per
/// opcode byte — the compiler unrolls the table at the assignment
...
const fn build_dispatch_table() -> [DslHandler; 256] {
    ...
    // (it's a `#[repr(u8)]` enum), so the index expression is valid
    // in const context.
    macro_rules! assign {
    ...
    // Hot opcodes (5): Move, Add, Jump, LoopHeader (warm), Return.
    ...
    // Warm opcodes (8): Jump8, JumpIfTrue/8, JumpIfFalse/8, LoopHeader,
    // Wide, ExtraWide.
    ...
    // Cold opcodes (140). All carry the `_dsl` suffix on their
    // proc-macro-emitted symbol. Listed in `Opcode` declaration order
    // so a discriminant audit reads top-to-bottom; the dispatch
    // table itself doesn't care about order.
    ...
    // -- arithmetic family (29 - 1 Add = 28 cold) --
    ...
    // -- control_flow family (10 - Hot/Warm pulled out = 2 cold) --
