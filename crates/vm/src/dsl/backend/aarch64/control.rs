...
///
/// This is a deliberately narrow frame-entry primitive. It requires an
/// asm-visible call-target table entry prepared by Rust at trampoline
/// entry / slow-path egress, and it bails to the normal semantic path
/// for unclassified callees, unsupported `this` coercions, stack
/// exhaustion, or missing frame-info storage.
#[macro_export]
macro_rules! call0_bytecode_or_branch {
    ($result:tt, $callee:tt, $this_reg:tt, $label:tt) => {
        concat!(
            // Preserve the result register operand before this macro's
            // capacity checks start using the operand scratch registers.
            "mov    w13, w",
            stringify!($result),
            "\n",
            // Callee must be the exact object recorded in the call-target table.
            "ldr    x0, [x20, x",
            stringify!($callee),
            ", lsl #3]\n",
            ...
            stringify!($label),
            "\n",
            // Load and, for global-this callees, coerce null/undefined to the global object.
            "ldr    x7, [x20, x",
            stringify!($this_reg),
            ", lsl #3]\n",
            ...
            "1:\n",
            // Depth, frame-info, and register-stack capacity checks.
            ...
            // Clear the callee frame to match Rust reserve_register_window semantics.
            "movz   x6, #0x1, lsl #32\n",
            ...
            stringify!($label),
            "\n",
            // Advance the caller frame-info PC to the post-Call0 bytecode.
            "sub    w3, w8, #1\n",
            ...
            // Populate the new callee frame info at index old_depth.
            ...
            // Switch pinned state to callee and dispatch its first opcode.
            "add    w8, w8, #1\n",
...
/// Replace the active frame with an eligible no-spread bytecode tail callee.
///
/// This mirrors the narrow `Call0` frame-entry path but keeps the
/// current frame depth and reuses the active register window base. The
/// hit case is intentionally limited to call sites marked no-spread in
/// the flat feedback metadata and frames marked recyclable by the
/// Rust-side ABI metadata; unsupported spread/cleanup cases branch to
/// the semantic path.
#[macro_export]
macro_rules! tail_call0_bytecode_or_branch {
    ($callee:tt, $this_reg:tt, $label:tt) => {
        concat!(
            // TailCall carries a 4-byte inline CallRange at PC+4:
            // count_lo/count_hi/base_lo/base_hi. Non-zero counts must
            // be proven no-spread via immutable call-site metadata in FV.
            "ldrh   w13, [x19, #4]\n",
            "ldrh   w14, [x19, #6]\n",
            "cbz    w13, 5f\n",
            "ldrh   w16, [x19, #8]\n",
            ...
            "\n",
            "sub    w16, w16, #1\n",
            "movz   x17, #({feedback_entry_stride} & 0xffff)\n",
            "movk   x17, #(({feedback_entry_stride} >> 16) & 0xffff), lsl #16\n",
            "movk   x17, #(({feedback_entry_stride} >> 32) & 0xffff), lsl #32\n",
            "movk   x17, #(({feedback_entry_stride} >> 48) & 0xffff), lsl #48\n",
            "mul    x16, x16, x17\n",
            "add    x17, x21, x16\n",
            "ldrb   w16, [x17, {feedback_call_flags}]\n",
            "tbz    w16, #{feedback_call_no_spread_bit}, ",
            stringify!($label),
            "\n",
            "ldrh   w16, [x17, {feedback_call_expected_arity}]\n",
            "cmp    w13, w16\n",
            "b.ne   ",
            stringify!($label),
            "\n",
            ...
            "ldr    x7, [x4, {call_target_global_this}]\n",
            "1:\n",
            // Active frame metadata must exist and be marked safe for
            // asm-side tail recycling.
            "ldr    w8, [x24, {state_frame_depth}]\n",
            "cbz    w8, ",
            stringify!($label),
            "\n",
            ...
            stringify!($label),
            "\n",
            "ldr    w17, [x3, {frame_info_frame_flags_raw}]\n",
            "tbnz   w17, #2, ",
            stringify!($label),
            "\n",
            "ldr    w0, [x3, {frame_info_register_len}]\n",
            "adds   w1, w14, w13\n",
            "b.cs   ",
            stringify!($label),
            "\n",
            ...
            stringify!($label),
            "\n",
            "add    x0, x20, x14, lsl #3\n",
            // Reuse the active register-window base for the callee frame.
            ...
            "\n",
            "add    x15, x14, x11, lsl #3\n",
            "ldr    w2, [x4, {call_target_parameter_count}]\n",
            "cmp    w13, w2\n",
            "csel   w2, w13, w2, lo\n",
            // Preserve caller metadata before overwriting this frame-info slot.
            ...
            "str    x17, [x15, x16, lsl #3]\n",
            "add    w16, w16, #1\n",
            "b      3b\n",
            "6:\n",
            // Clear the remaining replacement frame to match Rust reserve_register_window semantics.
            "movz   x6, #0x1, lsl #32\n",
            "movk   x6, #0x7ff8, lsl #48\n",
            "mov    w16, w2\n",
            "4:\n",
            ...
            "b      4b\n",
            "7:\n",
            // Overwrite the active frame info with the callee activation.
            "ldr    x6, [x4, {call_target_pb_base}]\n",
            "str    x6, [x3, {frame_info_pb_base}]\n",
            "str    x15, [x3, {frame_info_regs_base}]\n",
            "ldr    x16, [x4, {call_target_fv_base}]\n",
            ...
            // Switch pinned state to the replacement callee and dispatch its first opcode.
            "str    w12, [x24, {state_register_stack_top}]\n",
            "str    wzr, [x24, {state_pc}]\n",
            "ldr    x19, [x4, {call_target_pb_base}]\n",
            "str    x19, [x24, {state_pb}]\n",
