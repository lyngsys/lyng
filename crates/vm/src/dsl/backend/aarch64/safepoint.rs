...
//!
//! - `{vm_poll}` — `const VM_POLL_PENDING_OFFSET`.
...
///
/// The lowerer appends `, opcode_byte = N` to every `poll_safepoint!`
/// invocation. On the pending-poll branch, the macro emits
/// `inc_slow_safepoint_counter!(N)`. The fast path keeps its 2-instruction
/// shape regardless of feature flag state.
///
