...
pub struct Vm {
    ...
    /// Persistent asm-visible bytecode call target table keyed by
    /// `ObjectRef::get()`. Entries are populated when bytecode closures
    /// are allocated, so Call0/TailCall `LLInt` hit paths can check
    /// ordinary bytecode callees without rebuilding a full object scan at
    /// every slow-path boundary.
    pub(crate) llint_call_targets: Vec<crate::dsl::llint_state::LlIntCallTarget>,
...
impl Vm {
    ...
    #[inline]
    pub(crate) const fn register_stack_top(&self) -> usize {
    ...
    pub(crate) fn ensure_llint_register_stack_headroom(&mut self) {
        let target_len = self
            .register_stack_top
            .saturating_add(crate::dsl::llint_state::LLINT_REGISTER_STACK_HEADROOM);
    ...
    #[inline]
    pub(crate) fn llint_call_targets_for_entry(
        &self,
    ) -> (*const crate::dsl::llint_state::LlIntCallTarget, u32) {
        if self.debug_poll_enabled() {
            return (std::ptr::null(), 0);
        }
        (
            self.llint_call_targets.as_ptr(),
            u32::try_from(self.llint_call_targets.len()).unwrap_or(u32::MAX),
        )
    }

    pub(crate) fn update_llint_call_target(
        &mut self,
        agent: &Agent,
        object: ObjectRef,
        ...
        if self.llint_call_targets.len() <= index {
            self.llint_call_targets.resize(
                index + 1,
                crate::dsl::llint_state::LlIntCallTarget::default(),
            );
        }
        self.llint_call_targets[index] =
            crate::dsl::llint_state::llint_call_target_for_function(self, agent, object, data)
                .unwrap_or_default();
    }
