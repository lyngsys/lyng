...
impl Vm {
    ...
    pub(super) fn mirror_metadata_slot(&mut self, code: CodeRef, slot: FeedbackSlotId) {
        ...
        // Phase C Task 3.1: in debug builds, verify the MetadataTable entry
        // just written byte-matches the FeedbackEntry. Any divergence means
        // the dual-write has a bug.
        #[cfg(debug_assertions)]
        self.debug_assert_metadata_matches_flat(code, slot);
    }

    /// Phase C Task 3.1: debug-only equivalence assertion.
    ///
    /// Compares the `MetadataTable` `Property` entry for `slot` against the
    /// corresponding `FeedbackEntry` in `feedback_flat_storage`. Only the
    /// `Property` kind is checked — other kinds do not have a flat-storage
    /// counterpart with matching fields. Fires (via `debug_assert_eq!`) on any
    /// divergence, which means `mirror_metadata_slot` and `mirror_flat_slot`
    /// are writing different values and must be fixed before Phase D.
    ///
    /// Release builds compile this away entirely via the `cfg(not(debug_assertions))`
    /// no-op variant below.
    #[cfg(debug_assertions)]
    fn debug_assert_metadata_matches_flat(&self, code: CodeRef, slot: FeedbackSlotId) {
        ...
        // Only Property kind has a flat-storage projection counterpart today.
        // Other kinds will not byte-match because mirror_flat_slot only writes
        // FeedbackEntry's named-property fields.
        if kind != MetadataKind::Property {
            return;
        }

        let Some(table) = self.metadata_table(code) else {
            return;
        };
        let idx = code_index(code);
        let Some(flat_box) = self.feedback_flat_storage.get(idx) else {
            return;
        };
        let zero_based = (slot.get() - 1) as usize;
        let Some(flat_entry) = flat_box.get(zero_based) else {
            return;
        };
        ...
        debug_assert_eq!(
            table_entry.generation,
            flat_entry.generation,
            "MetadataTable.property.generation != FeedbackEntry.generation (code={code:?}, slot={slot:?})"
        );
        debug_assert_eq!(
            table_entry.handler_bits,
            flat_entry.named_handler_bits,
            "MetadataTable.property.handler_bits != FeedbackEntry.named_handler_bits (code={code:?}, slot={slot:?})"
        );
        debug_assert_eq!(
            table_entry.aux_bits,
            flat_entry.named_aux_bits,
            "MetadataTable.property.aux_bits != FeedbackEntry.named_aux_bits (code={code:?}, slot={slot:?})"
        );
    }
