...
impl<'a> VmDebugPauseContext<'a> {
    ...
    /// The referrer from the `Vm` side-stack at the current live pause.
    #[cfg(test)]
    pub(crate) fn current_referrer(&self) -> Option<lyng_common::AtomId> {
        self.vm.current_referrer()
    }

    /// Exposes the Agent's ambient `running_context` and the VM's frame-derived
    /// values together for regression coverage (test-only).
    #[cfg(test)]
    pub(crate) fn running_context_parity(&self) -> RunningContextParity {
        RunningContextParity {
            scalar: self.agent.running_context(),
            frame_realm: self.vm.current_realm_of(self.agent),
...
#[cfg(test)]
pub struct RunningContextParity {
