...
use super::{
    ArrayFamilyBuiltins, ArrayFamilyPrototypes, FamilyInstallContext,
...
use lyng_env::Agent;
use lyng_types::{
    BuiltinFunctionId, ObjectRef, RealmRef, Value, WellKnownSymbolId, array_at_builtin,
    array_builtin, array_concat_builtin, array_copy_within_builtin, array_entries_builtin,
    array_every_builtin, array_fill_builtin, array_filter_builtin, array_find_builtin,
    array_find_index_builtin, array_find_last_builtin, array_find_last_index_builtin,
    array_flat_builtin, array_flat_map_builtin, array_for_each_builtin, array_from_async_builtin,
    array_from_builtin, array_includes_builtin, array_index_of_builtin, array_is_array_builtin,
...
fn install_array_iterator_prototype_descriptors(
    ...
    let next = agent.atoms_mut().intern("next");
...
impl ArrayDescriptorAtoms {
    fn new(agent: &mut Agent) -> Self {
        ...
        Self {
            ...
            copy_within: atoms.intern("copyWithin"),
            entries: atoms.intern("entries"),
            every: atoms.intern("every"),
            fill: atoms.intern("fill"),
            filter: atoms.intern("filter"),
            find: atoms.intern("find"),
            find_index: atoms.intern("findIndex"),
            find_last: atoms.intern("findLast"),
            find_last_index: atoms.intern("findLastIndex"),
            flat: atoms.intern("flat"),
            flat_map: atoms.intern("flatMap"),
            for_each: atoms.intern("forEach"),
            from: atoms.intern("from"),
            from_async: atoms.intern("fromAsync"),
            includes: atoms.intern("includes"),
            index_of: atoms.intern("indexOf"),
            is_array: atoms.intern("isArray"),
            join: atoms.intern("join"),
            keys: atoms.intern("keys"),
            last_index_of: atoms.intern("lastIndexOf"),
            map: atoms.intern("map"),
            of: atoms.intern("of"),
            pop: atoms.intern("pop"),
            push: atoms.intern("push"),
            reduce: atoms.intern("reduce"),
            reduce_right: atoms.intern("reduceRight"),
            reverse: atoms.intern("reverse"),
            shift: atoms.intern("shift"),
            slice: atoms.intern("slice"),
            some: atoms.intern("some"),
            sort: atoms.intern("sort"),
            splice: atoms.intern("splice"),
            to_locale_string: atoms.intern("toLocaleString"),
            to_reversed: atoms.intern("toReversed"),
            to_sorted: atoms.intern("toSorted"),
            to_spliced: atoms.intern("toSpliced"),
            unshift: atoms.intern("unshift"),
            values: atoms.intern("values"),
            with: atoms.intern("with"),
        }
    }
}
