...
use lyng_objects::FunctionObjectData;
...
fn dispatch_proxy_builtin<Cx: PublicBuiltinDispatchContext>(
    ...
    if entry == lyng_types::proxy_builtin() {
        return proxy_builtin(context, invocation).map(Some);
    }
    if entry == lyng_types::proxy_revocable_builtin() {
        return proxy_revocable_builtin(context, invocation).map(Some);
    }
    if entry == lyng_types::proxy_revoke_builtin() {
...
fn proxy_revocable_builtin<Cx: PublicBuiltinDispatchContext>(
    ...
    let revoke = cx.allocate_builtin_function(lyng_types::proxy_revoke_builtin())?;
