...
fn dictionary_transition_via_redefine_fires_watchpoint() {
    ...
    let original_shape = agent.heap().view().object(obj).unwrap().shape().unwrap();
...
fn dictionary_transition_via_delete_fires_watchpoint() {
    ...
    let key = PropertyKey::from_atom(AtomId::from_raw(1));
    agent
        .delete(obj, key, &mut NoopAdaptiveProtoLoadDispatch)
...
fn dictionary_transition_via_property_overflow_fires_watchpoint() {
    ...
    let brink_shape = agent.heap().view().object(obj).unwrap().shape().unwrap();
...
fn set_prototype_of_allocates_fresh_shape() {
    ...
    agent
        .set_prototype_of(obj, Some(proto), &mut NoopAdaptiveProtoLoadDispatch)
        .unwrap();
...
fn prototype_transition_table_dedups() {
    ...
    let s_a_before = agent.heap().view().object(obj_a).unwrap().shape().unwrap();
    ...
    agent
        .set_prototype_of(obj_a, Some(proto), &mut NoopAdaptiveProtoLoadDispatch)
        .unwrap();
    agent
        .set_prototype_of(obj_b, Some(proto), &mut NoopAdaptiveProtoLoadDispatch)
        .unwrap();
...
fn null_and_object_prototype_produce_distinct_shapes() {
    ...
    agent
        .set_prototype_of(obj_a, None, &mut NoopAdaptiveProtoLoadDispatch)
        .unwrap();
    agent
        .set_prototype_of(obj_b, Some(proto), &mut NoopAdaptiveProtoLoadDispatch)
        .unwrap();
...
fn dead_prototype_transition_entry_pruned_on_gc() {
    ...
    agent
        .set_prototype_of(obj1, Some(proto1), &mut NoopAdaptiveProtoLoadDispatch)
        .unwrap();
    agent
        .set_prototype_of(obj2, Some(proto2), &mut NoopAdaptiveProtoLoadDispatch)
        .unwrap();
