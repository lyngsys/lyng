//! Process-level integration tests for the `lyng --test262` flag.
//!
...
fn test262_flag_prints_to_stdout() {
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_lyng"))
        .arg("--test262")
        .arg(script.path())
        .output()
        .expect("lyng binary should run");

    assert!(
        output.status.success(),
        "lyng --test262 exited with {:?}, stderr: {}",
        output.status,
...
fn default_cli_leaves_test262_undefined() {
    ...
    assert!(
        output.status.success(),
        "default lyng exited with {:?}, stderr: {}",
        output.status,
        String::from_utf8_lossy(&output.stderr)
    );
}
...
impl TempScript {
    fn new(label: &str, source: &str) -> Self {
        let unique = SystemTime::now()
            ...
            .as_nanos();
        let path = std::env::temp_dir().join(format!("lyng-test262-{unique}-{label}"));
