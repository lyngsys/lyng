...
impl FunctionCompiler<'_, '_> {
    pub(super) fn lower_template_literal(
        ...
        for (index, expression) in expressions.iter().copied().enumerate() {
            let coerced = self.lower_internal_template_to_string(expr_id, expression)?;
    ...
    fn lower_tagged_template_call(
        ...
        let expressions = self.ast().templates().get_expressions(template).to_vec();
        let argument_count = u16::try_from(expressions.len() + 1)
            .map_err(|_| LoweringError::UnsupportedExpression { expr: expr_id })?;
        let argument_range = self.reserve_argument_block(argument_count)?;
        let base = argument_range.argument_base();
        self.lower_template_object_into(span, template, base)?;
        for (index, expression) in expressions.into_iter().enumerate() {
            let target = base + 1 + u16::try_from(index).unwrap_or(u16::MAX);
    ...
    fn lower_template_object_into(
        ...
        let argument_count = u16::try_from(1 + quasis.len() * 2).map_err(|_| {
        ...
        let argument_range = self.reserve_argument_block(argument_count)?;
        let base = argument_range.argument_base();
        self.emit_load_i32_constant(base, i32::try_from(template.raw()).unwrap_or(i32::MAX))?;
        for (index, quasi) in quasis.iter().enumerate() {
            let cooked_slot = base + 1 + u16::try_from(index * 2).unwrap_or(u16::MAX);
            if let Some(cooked_id) = quasi.cooked {
                self.emit_load_string(cooked_slot, cooked_id)?;
            } else {
                self.emit_load_undefined(cooked_slot)?;
            }
            let raw_slot = cooked_slot + 1;
            self.emit_load_string(raw_slot, quasi.raw)?;
        }
    ...
    fn lower_internal_template_to_string(
        ...
        self.lower_expr_into(expression, argument_range.argument_base())?;
