// DSL-1 Phase 1.C.2 op_bit_and_dsl handler asm (AArch64), captured from
//   cargo rustc --release -p lyng-js-vm --lib -- --emit=asm
//
// Symbol: lyng_js_vm::dsl::handlers::cold::op_bit_and_dsl
...
// Fast-path instruction count (from `op_bit_and_dsl:` through
// `bl _op_bit_and_record_smi_rs` inclusive): 35 instructions.
//
// Mirrors op_sub's shape byte-for-byte except `and w15, w13, w14 +
// sxtw x15, w15` (the 2-instruction `bit_and_smi!` body) replaces
// the 3-instruction `sub_smi_overflow!` body (subs + b.vs + sxtw).
// Bitwise AND on tagged ints cannot overflow, so there is no b.vs
// branch — 1 fewer instruction in the fast path.
