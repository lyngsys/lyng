// DSL-1 Phase 1.C.2 op_shift_left_dsl handler asm (AArch64), captured from
//   cargo rustc --release -p lyng-js-vm --lib -- --emit=asm
//
// Symbol: lyng_js_vm::dsl::handlers::cold::op_shift_left_dsl
...
op_shift_left_dsl:
    ...
    mov     x0, x24                      ; a0 = STATE
    mov     w1, w12                      ; a1 = slot
    bl      _op_shift_left_record_smi_rs
...
// Fast-path instruction count (from `op_shift_left_dsl:` through
// `bl _op_shift_left_record_smi_rs` inclusive): 36 instructions.
//
// Mirrors op_bit_and's shape byte-for-byte except `and w16, w14, #0x1f +
// lsl w15, w13, w16 + sxtw x15, w15` (the 3-instruction `shift_left_smi!`
// body) replaces the 2-instruction `bit_and_smi!` body (and + sxtw).
// The extra `and w16, w14, #0x1f` masks the RHS shift count to its low 5
// bits per ECMAScript `<<` / ToUint32 semantics — JSC's op_lshift uses
// the same `and #0x1f + lsl` pattern.
