...
The asm-visible [`LlIntState`](../../../crates/lyng/vm/src/dsl/llint_state.rs) has no `frame_const_base` field. The constants array is not a flat `*const Value` — it lives behind several pointer hops as a `Vec<ConstantValue>` enum, where the `ConstantValue::Atom` and `ConstantValue::Builtin` variants require runtime heap-allocation or builtin lookup to materialize a `Value`. Neither is feasible inline in asm.
...
`ConstantValue` enum definition: [crates/lyng/bytecode/src/metadata.rs:152-159](../../../crates/lyng/bytecode/src/metadata.rs).
...
1. **Asm-visible `frame_const_base: *const Value` on [`LlIntState`](../../../crates/lyng/vm/src/dsl/llint_state.rs)** — pointer to a precomputed flat `Value` array of resolved constants.
2. **`LLINT_STATE_FRAME_CONST_BASE_OFFSET` const in [`reg_convention.rs`](../../../crates/lyng/vm/src/dsl/reg_convention.rs)** — exposed via `offset_of!` for the DSL backend.
...
4. **Update the entry shim** ([`vm/src/dsl/entry.rs`](../../../crates/lyng/vm/src/dsl/entry.rs)) to populate the new field on `Vm::run` entry.
5. **Update the call/return slow paths** ([`vm/src/dsl/slow_path.rs`](../../../crates/lyng/vm/src/dsl/slow_path.rs)) to refresh the field on frame transitions, mirroring the existing PB/REGS/FV refresh discipline.
...
Cold-stub at [`crates/lyng/vm/src/dsl/handlers/cold.rs`](../../../crates/lyng/vm/src/dsl/handlers/cold.rs) remains in place; `op_load_const8` continues to dispatch through the slow path during Phase 1.A.
