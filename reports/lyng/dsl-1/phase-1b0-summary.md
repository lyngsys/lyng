...
- **Register clobber bug surfaced and fixed:** initial slow-path counter macros used `x9`/`x10` which conflict with live operands after the decode prologue. Fixed in Task 5 by switching to `x16`/`x17` (AAPCS64 IP0/IP1, reloaded by `call_slow!`'s bridge). Documented in [counters.rs](../../../crates/vm/src/dsl/backend/aarch64/counters.rs).
