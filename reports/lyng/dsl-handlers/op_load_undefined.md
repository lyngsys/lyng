...
op_load_undefined_dsl:
    ...
    mov     x11, #4294967296            ; tag_undefined!(t0) — movz x11, #0x1, lsl #32
    movk    x11, #32760, lsl #48        ; tag_undefined!(t0) — movk x11, #0x7ff8, lsl #48
    str     x11, [x20, x9, lsl #3]      ; store_reg!(a, t0) — REGS[a] := undefined
...
Note: the LLVM assembler rewrote the canonical `tag_undefined!` shifted-
immediate form
(`movz x11, #0x1, lsl #32 ; movk x11, #0x7ff8, lsl #48`) to the
mathematically-equivalent `mov x11, #4294967296 ; movk x11, #32760, lsl #48`
(where 4294967296 = 0x1_0000_0000 and 32760 = 0x7ff8). The encoding is
...
The closest LLInt analog to "write `undefined` to a register" is the
`move ValueUndefined, t0` + `storeq t0, [...]` idiom used inline
...
(which dispatches on the operand encoding). The `undefined` value lives
...
dest, dispatch". Lyng's `op_load_undefined` short-circuits the constant-
table indirection by materializing `Value::undefined()` directly with a
2-instruction `movz/movk` sequence — no memory access for the value.

A more directly-comparable LLInt fragment is the `move ValueUndefined,
t0; storeq t0, [...]` pattern used at LowLevelInterpreter64.asm lines
333, 391, 760, 799, 836 — that idiom is two instructions to materialize
+ one to store, matching lyng's tag-and-store fast path exactly.
...
| Step | Lyng DSL                                         | LLInt (`op_mov`-via-undefined-constant)             |
|------|--------------------------------------------------|-----------------------------------------------------|
...
| 3    | `movz x11, #0x1, lsl #32` (tag_undefined low)    | `loadConstantOrVariable(size, t1, t2)` — table load |
| 4    | `movk x11, #0x7ff8, lsl #48` (tag_undefined high)| (rolled into step 3 as a single 64-bit `loadq`)     |
...
- **Value layout.** Lyng uses NaN-tagged 64-bit `Value` (high 16 bits = tag,
  low 32 bits = payload, canonical-NaN prefix in bits 48-60). `undefined`
  is the constant `0x7ff8_0001_0000_0000`, requiring a 2-instruction
  `movz/movk` to materialize. JSC's JSValue is also NaN-boxed in 64-bit
  mode but uses `ValueUndefined` as a single-instruction constant move
  on x86_64 (`movabsq $imm, t0`) and a similar 2-instruction `movz/movk`
  on AArch64 — parity, not a delta.
- **No constant-table indirection.** Lyng has a dedicated `LoadUndefined`
...
- **Unused `bx` decode.** Lyng's `Abx` layout reads a 16-bit operand at
  `[PC+2]` even though `op_load_undefined` doesn't use it. This `ldrh`
...
Microbench snippet not yet present for `LoadUndefined`; deferred to
...
Post-port V8 v7 7-sample geomean: see `/tmp/post-task1-v8.json`. A
single-opcode port is not expected to move the V8 v7 geomean
measurably — the LoadUndefined dispatch share is sub-percent on the
...
port value: **0%** — the slow-path shim `op_load_undefined_slow_rs`
was deleted alongside this port; the opcode has no fail mode (it
unconditionally writes `Value::undefined()` to register `a` and
dispatches).
...
- **Slow-path shim deleted.** `op_load_undefined_slow_rs` was the
...
- **Manifest entry unchanged.** `crates/lyng-js/vm/src/dsl/opcode_manifest.rs:84`
