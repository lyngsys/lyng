# `op_load_true` DSL port (Phase 1.A, Task 3)

Third Phase-1.A port — mirrors Tasks 1/2 (`op_load_undefined`, `op_load_null`)
but uses `tag_bool_const!` instead of `tag_undefined!`/`tag_null!`. The handler
writes `Value::from_bool(true)` to register `a`; the `bx` operand is unused
(layout reserves the slot for forward compat).

Bool tag carries an explicit payload bit (`#0x1` for `true`, `#0x0` for
`false`), so `tag_bool_const!` produces **3 instructions** rather than the
2 instructions used by `tag_undefined!`/`tag_null!` (whose payloads are
folded into the same `movz` as their kind tag).
...
See `reports/js/lyng-js/dsl-asm-baseline-aarch64/op_load_true.asm`.
...
op_load_true_dsl:
    ...
    movk    x11, #32760, lsl #48        ; tag_bool_const!(t0, 1) — movk x11, #0x7ff8, lsl #48
...
**10 instructions** total — one more than `op_load_undefined`/`op_load_null`
(9 each). The +1 comes from `tag_bool_const!`'s 3-instruction expansion: the
Bool tag needs both an explicit payload (`#0x1`) and an explicit kind
(`#0x3`) materialized in different bit ranges, so it cannot fold them into a
single `movz` the way `tag_undefined!`/`tag_null!` do (their payloads share
the `lsl #32` slot with the kind byte). LLVM did not elide the unused `ldrh`
decode for `_bx`; the encoding is small (4 bytes) and proving the load is
dead requires alias analysis on the bytecode buffer that LLVM doesn't
perform here.

Note: the LLVM assembler rewrote the canonical `tag_bool_const!` form
(`movz x11, #0x1 ; movk x11, #0x3, lsl #32 ; movk x11, #0x7ff8, lsl #48`)
to the equivalent (`mov x11, #1 ; movk x11, #3, lsl #32 ; movk x11, #32760, lsl #48`).
...
The closest LLInt analog to "write `true` to a register" is the
`move ValueTrue, t0` + `storeq t0, [...]` idiom used inline within
...
(which dispatches on the operand encoding). The `true` value lives in
...
dest, dispatch". Lyng's `op_load_true` short-circuits the constant-
table indirection by materializing `Value::from_bool(true)` directly
with a 3-instruction `movz/movk/movk` sequence — no memory access for
the value.

A more directly-comparable LLInt fragment is the `move ValueTrue, t0;
storeq t0, [...]` pattern used inline within several handlers — that
idiom is one instruction to materialize (because `ValueTrue` fits in a
...
| Step | Lyng DSL                                         | LLInt (`op_mov`-via-true-constant)                  |
|------|--------------------------------------------------|-----------------------------------------------------|
...
| 3    | `movz x11, #0x1` (Bool payload)                  | `loadConstantOrVariable(size, t1, t2)` — table load |
...
- **Value layout.** Lyng uses NaN-tagged 64-bit `Value` (high 16 bits = tag,
  low 32 bits = payload, canonical-NaN prefix in bits 48-60). `true` is the
  constant `0x7ff8_0003_0000_0001` (payload `#0x1`, kind `#0x3` for Bool),
  ...
  shifted-`movz` as the kind tag — `Bool` has a non-zero payload byte that
  must be materialized separately. JSC's JSValue is also NaN-boxed in
  64-bit mode and uses `ValueTrue` as a single-instruction constant move
  on x86_64 (`movabsq $imm, t0`) and a 3-instruction `movz/movk/movk` on
  AArch64 — parity, not a delta.
- **No constant-table indirection.** Lyng has a dedicated `LoadTrue`
...
- **Unused `bx` decode.** Lyng's `Abx` layout reads a 16-bit operand at
  `[PC+2]` even though `op_load_true` doesn't use it. This `ldrh` is the
...
Microbench snippet not yet present for `LoadTrue`; deferred to
...
measurably — the LoadTrue dispatch share is sub-percent on the
...
port value: **0%** — the slow-path shim `op_load_true_slow_rs`
was deleted alongside this port; the opcode has no fail mode (it
unconditionally writes `Value::from_bool(true)` to register `a` and
dispatches).
...
- **Slow-path shim deleted.** `op_load_true_slow_rs` was the
  cold-stub bridge to `op_load_true_semantic`; with the inline fast
...
- **Manifest entry unchanged.** The opcode manifest references
  `op_load_true_dsl` (the handler symbol, not the shim). The symbol
  identity is preserved; only the body changed.
- **Same shape as Tasks 1/2, +1 instruction.** The body template
  (`tag_X!(t0); store_reg!(a, t0); dispatch!()`) is identical to
  `op_load_undefined`/`op_load_null`. The only divergence is the
  3-instruction `tag_bool_const!` expansion vs the 2-instruction
  `tag_undefined!`/`tag_null!`. Same dispatch tail; same unused-bx
  decode.
